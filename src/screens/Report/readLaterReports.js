...
import { getReadLaterReports, removeReadLaterReport } from '../../redux/actions/reportsActions';

class ReadLaterReports extends Component {
  componentDidMount() {
    this.props.getReadLaterReports(); 
  }
  ...
  render() {
    return (
      <div>
        ...
        {
          this.props.readLaterReports.map(report => (
            <div class="card">
              <div class="card-body">
                <h3><a href="{{ route('dashboard.report.read', $report->slug) }}">{ report.title }</a></h3>
                <p class="citation">citation: { report.citation }</p>
                <p>{ report.excerpt }</p>
                <button  class="btn btn-danger" onClick={() => this.props.removeReadLaterReport(report.id)}><i class="fas fa-trash"></i> Remove from list</button>
              </div>
            </div>
          ))
        }
...
const mapStateToProps = ({ reports }) => ({
  readLaterReports: reports.readLaterReports
});
